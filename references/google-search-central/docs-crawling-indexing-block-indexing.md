---
title: "Block Search Indexing with noindex | Google Search Central | Documentation | Google for Developers"
source_url: https://developers.google.com/search/docs/crawling-indexing/block-indexing
...
rule, such as Google. When Googlebot crawls that page and
...
rule with other rules that control indexing. For
example, you can join a `nofollow`
...
rule from indexing
a page on your site, place the following `<meta>`
...
rule differently. As a result, it is possible that your page might
...
files. Here's an example of an HTTP response with an `X-Robots-Tag`
...
tags and HTTP headers. If
a page is still appearing in results, it's probably because we haven't crawled the page since
you added the `noindex`

rule. Depending on the importance of the page on the
...
rule is visible to Googlebot. To test if your
`noindex`

implementation is correct, use the
[URL Inspection tool](https://support.google.com/webmasters/answer/9012289)
