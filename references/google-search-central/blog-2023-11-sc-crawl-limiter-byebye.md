---
title: "Upcoming deprecation of Crawl Rate Limiter Tool in Search Console | Google Search Central Blog | Google for Developers"
source_url: https://developers.google.com/search/blog/2023/11/sc-crawl-limiter-byebye
...
status codes for a range of URLs, Googlebot will automatically, and almost
...
In contrast, the rate limiter tool had a much slower effect; in fact it may have taken over a day for the new limits to be applied on crawling. Fortunately though, site owners rarely had to resort to using the tool, and those who have, in many cases set the crawling speed to the bare minimum. With the deprecation of the crawl limiter tool, we're also setting the minimum crawling speed to a lower rate, comparable to the old crawl rate limits. This means that we effectively continue honoring the settings that some site owners have set in the past if the Search interest is low, and our crawlers don't waste the site's bandwidth.
