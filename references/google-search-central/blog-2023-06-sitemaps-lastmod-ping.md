---
title: "Sitemaps ping endpoint is going away | Google Search Central Blog | Google for Developers"
source_url: https://developers.google.com/search/blog/2023/06/sitemaps-lastmod-ping
...
element to be useful, first it needs to be in a supported date format
...
element that it changed yesterday, eventually we're not going to believe you
anymore when it comes to the last modified date of your pages.

You can use a `lastmod`

element for all the pages in your sitemap, or just the ones
...
the site. In these cases it's fine to leave out `lastmod`
...
don't have to update the `lastmod`
...
Going on a small tangent, if you look at the `xmlns`

attribute in the sitemap snippet,
...
Google still doesn't use the `changefreq`
...
specifically is also conceptually overlapping with `lastmod`

.
The `priority`

element is a heavily subjective field and based on our internal studies,
