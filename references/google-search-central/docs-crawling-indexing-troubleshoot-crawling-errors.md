---
title: "Troubleshoot Google Search Crawling Errors | Google Search Central | Documentation | Google for Developers"
source_url: https://developers.google.com/search/docs/crawling-indexing/troubleshoot-crawling-errors
category: google-search-central
section: "Google Search Central — official technical SEO + structured data docs"
date: 2025-12-18
---

# Troubleshoot Google Search Crawling Errors | Google Search Central | Documentation | Google for Developers
...
[See if Googlebot is encountering availability issues on your site](#availability_issues).[See whether you have pages that aren't being crawled, but should be](#not_crawled_should_be).[See whether any parts of your site need to be crawled more quickly than they already are.](#updates)[Improve your site's crawl efficiency.](#improve_crawl_efficiency)[Handle overcrawling of your site](#emergencies).
...
[Googlebot](/search/docs/crawling-indexing/overview-google-crawlers). Whether or
...
maximum serving capacity, or you are [out of crawl budget](/crawling/docs/crawl-budget#more-crawl-budget).
...
- Review your crawling priorities (a.k.a. use your crawl budget wisely).
[Manage your inventory](/crawling/docs/crawl-budget#manage_inventory)and[improve your site's crawling efficiency](#improve_crawl_efficiency). [Check that you're not running out of serving capacity](#availability_issues). Googlebot will scale back its crawling if it detects that your servers are having trouble responding to crawl requests.
...
Examine your site logs to see when specific URLs were crawled by [Googlebot](/search/docs/crawling-indexing/overview-google-crawlers).
...
- Use a
[news sitemap](/search/docs/crawling-indexing/sitemaps/news-sitemap)if your site has news content. - Use the
`<lastmod>`

tag in sitemaps to indicate when an indexed URL has been updated. - Use a
[crawlable URL structure](/search/docs/crawling-indexing/url-structure)to help Google find your pages. - Provide standard,
[crawlable](/search/docs/crawling-indexing/links-crawlable#crawlable-links)to help Google find your pages.`<a>`

links - If your site uses separate HTML for mobile and desktop versions, provide the same set of links
...
[sitemap](/search/docs/crawling-indexing/sitemaps/overview)file. Google only[indexes the mobile version](/search/docs/crawling-indexing/mobile/mobile-sites-mobile-first-indexing)of pages, and limiting the links shown there can slow down discovery of new pages.
...
- Including URLs in your sitemaps that
[you don't want to appear in Search](#hide_urls). This can waste your crawl budget on pages that you don't want indexed.
...
[AdsBot](/search/docs/crawling-indexing/overview-google-crawlers#adsbot)is more likely to set the
...
HTTP request
headers). If our crawlers send the `If-Modified-Since`

header, the header's value
is the [date and time](https://www.rfc-editor.org/rfc/rfc9110#name-if-modified-since)the content was last crawled. Based on that value, the server may choose to return a

`304 (Not Modified)`

HTTP status code with no response body, in which case Google
will reuse the content version it crawled the last time. If the content is newer than the date
specified by the crawler in the `If-Modified-Since`

header, the server can return a
`200 (OK)`

HTTP status code with the response body.
Independently of the request headers, you can send a `304 (Not Modified)`
...
-
[Faceted navigation](/search/docs/crawling-indexing/crawling-managing-faceted-navigation)and[session identifiers](/search/blog/2007/09/google-duplicate-content-caused-by-url): Faceted navigation is typically duplicate content from the site; session identifiers and other URL parameters that simply sort or filter the page don't provide new content. Learn how to[manage crawling of faceted navigation pages](/search/docs/crawling-indexing/crawling-managing-faceted-navigation). [Duplicate content](/search/docs/crawling-indexing/consolidate-duplicate-urls): Help Google identify duplicate content to avoid unnecessary crawling.: Return a`soft 404`

pages`404`

code when a page no longer exists.[Hacked pages](/search/docs/monitor-debug/security/what-is-hacked): Be sure to check the[Security Issues report](https://search.google.com/search-console/security-issues)and fix or remove any hacked pages you find.[Infinite spaces](/search/blog/2008/08/to-infinity-and-beyond-no)and proxies: Block these from crawling with robots.txt.[Low quality and spam content](/search/docs/essentials/spam-policies): Good to avoid, obviously.- Shopping cart pages, infinite scrolling pages, and pages that perform an action (such as "sign up" or "buy now" pages).
...
status codeSuch pages may be generated for various reasons by your website's web server or content management system, or the user's browser. For example:
...
[The page and content are no longer available.](#pagegone)[The page or content is now somewhere else.](#pagemoved)[The page and content still exist.](#pageother)
...
[ 301 (permanent redirect)](/search/docs/crawling-indexing/301-redirects)
...
[URL Inspection tool](https://support.google.com/webmasters/answer/9012289)to verify whether your URL is actually returning the correct code.
...
[robots.txt](/search/docs/crawling-indexing/robots/intro)), having too many
