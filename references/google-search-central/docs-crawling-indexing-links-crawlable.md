---
title: "SEO Link Best Practices for Google | Google Search Central | Documentation | Google for Developers"
...
Generally, Google can only crawl your link if it's an `<a>`

HTML element (also
known as *anchor element*)
with an `href`

attribute. Most links in other formats won't be parsed and extracted
by Google's crawlers. Google can't reliably extract URLs from `<a>`

elements
that don't have an `href`

attribute or other tags that perform as links because of
...
Make sure that the URL in your `<a>`

element resolves into an actual web
...
attribute as anchor text if the
`<a>`
...
For images as links, Google uses the `alt`
...
reputation from your link. This would be a good time to use
...
. If users can insert links on your site (for example, you have a
forum section or Q&A site), add [or](/search/docs/crawling-indexing/qualify-outbound-links#ugc)
