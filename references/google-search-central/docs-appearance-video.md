...
source_url: https://developers.google.com/search/docs/appearance/video
category: google-search-central
section: "Google Search Central — official technical SEO + structured data docs"
date: 2009-11-05
---

# Video SEO Best Practices | Google Search Central | Documentation | Google for Developers
...
Google, including the main search results page, Video mode, Google Images, and [Discover](/search/docs/appearance/google-discover):

Optimize your videos to appear on Google by following these best practices:

[Help Google find your videos](#help-google-find)[Ensure your videos can be indexed](#indexing-criteria)[Enable specific video features](#enable-search-features)[Remove, restrict, or update your videos as needed](#remove)[Monitor your videos with Search Console](#monitor)[Troubleshoot video issues](#troubleshoot)

## Help Google find your videos

The [technical requirements](/search/docs/essentials/technical) for getting your
...
element. - Don't use
[fragment identifiers](https://wikipedia.org/wiki/URI_fragment)to load the video, as Google Search doesn't generally support URL fragments. - If you're using JavaScript to inject the video, make sure that it
[appears in the rendered HTML in the URL inspection tool](/search/docs/crawling-indexing/javascript/javascript-seo-basics). - If you're using a media API (for example, the
[Media Source API](https://developer.mozilla.org/en-US/docs/Web/API/Media_Source_Extensions_API)), make sure the HTML video container element is still injected even if the media API call fails (in addition to providing metadata about the video). That way Google can still locate the position of the video container even if there's an issue with calling the media API. - Don't rely on user actions (such as swiping, clicking, or typing) to load the video.

To make it easier for Google to find your videos, we recommend providing metadata about the video.
We support [structured data](/search/docs/appearance/structured-data/video),
[video sitemaps](/search/docs/crawling-indexing/sitemaps/video-sitemaps), and the
...
- The
[watch page](#watch-page)must be indexed. - The indexed watch page must be performing well in Search before its video can be considered for indexing.
- The video must be embedded on a watch page.
- The video can't be hidden behind other elements.
- The video must have a
[valid thumbnail](#valid-thumbnail)that's available at a[stable URL](#stable-url).
...
To make your videos [eligible for specific features](#enable-search-features) like key
...
you can [verify Googlebot](/search/docs/crawling-indexing/verifying-googlebot) before
displaying a stable version of your media URLs. For example, you can choose to serve the
[ contentUrl](/search/docs/appearance/structured-data/video#content-url)
...
mode, [Key Moments](/search/docs/appearance/video#key-moments), the
[Live Badge](/search/docs/appearance/video#live-badge), and other rich formats),
...
check our best practices for writing good [titles](/search/docs/appearance/title-link#page-titles)
and [descriptions](/search/docs/appearance/snippet#meta-descriptions).
...
[video indexing criteria](#indexing-criteria).

For your own watch page where you've embedded the third-party player, we still recommend that you
[provide structured data](#structured-data), and you may
also include these pages in your [video sitemaps](/search/docs/crawling-indexing/sitemaps/video-sitemaps).
To be eligible for more video features, check with your video host to ensure they allow Google to
[fetch your video file](#allow-fetch).
...
this URL is the value for the <?xml version="1.0" encoding="UTF-8"?> <urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9" xmlns:video="http://www.google.com/schemas/sitemap-video/1.1"> <url> <loc> |
2. Video player
|
The URL of a specific player for the video. This is often the <iframe src="
If you're using structured data, provide the video player URL as the value for the
"embedUrl": " If you're using a video sitemap, provide the video player URL as the value for the <video:player_loc> |
3. Video file
|
The URL of the video file's actual content bytes, which could be hosted on the embedding site, a CDN, or on a streaming service. In the <object data=" In the <video controls width="250"> <source src=" In the <embed type="video/webm" src="
If you're using structured data, provide the video file's URL as the value for the
"contentUrl": " If you're using a video sitemap, provide the video file's URL as the value for the
<video:content_loc> |
...
[allow Google to fetch your video files](#allow-fetch), Google will try to
...
HTML element, specify the`poster`

attribute. - In a video sitemap (including mRSS), specify the
`<video:thumbnail_loc>`

tag (or`<media:thumbnail>`

respectively). - For structured data, specify the

property.`thumbnailUrl`

- For
[OGP](https://ogp.me/), specify the`og:video:image`
...
**Transparency**### Provide consistent and unique information in your structured data

To influence how your videos appear on Google, describe your video with
[structured data](/search/docs/appearance/structured-data/video). Ensure that any
...
properties for each video on your site.
...
[allow Google to fetch your video files](#allow-fetch). You can set the maximum
duration for these video previews using the [ max-video-preview](/search/docs/crawling-indexing/robots-meta-tag#max-video-preview)
robots
...
**If your video is embedded on your web page or you're running a video platform**, there are two ways that you can enable key moments:: Specify the exact start and end time to each segment, and what label to display for each segment. This is supported in all languages where Google Search is available.`Clip`

structured data: Tell Google where timestamps typically go in your URL structure, so that Google can automatically identify key moments and link users to those points within the video. This is supported for the following languages: English, Spanish, Portuguese, Italian, Chinese, French, Japanese, German, Turkish, Korean, Dutch, and Russian.`SeekToAction`
...
[ BroadcastEvent
structured data](/search/docs/appearance/structured-data/video#broadcast-event).
...
[video previews](#video-previews) and [key moments](#key-moments).
...
the actual video bytes with the
rule or`noindex`

[robots.txt](/search/docs/crawling-indexing/robots/intro)file. - The video file must be available at a
[stable URL](#stable-url). - Use structured data to provide the
value of a`contentURL`

[supported file type](#supported-video-files). - The host of the video watch page and the server that's streaming the actual video must have
enough server resources to support crawling. So if your landing page at
`example.com/puppies.html`

has an embedded puppies video served by`streamserver.example.com`

, both`example.com`

and`streamserver.example.com`

must meet the[technical requirements for Google Search](/search/docs/essentials/technical)and have[available server capacity](/search/docs/crawling-indexing/troubleshoot-crawling-errors#availability_issues).
...
**Return a**for any watch page that's embedding the removed or expired video. In addition to the`404 (Not found)`

`404`

response code, you can still return the HTML of the page to make the change transparent to most users.**Include a**on any watch page that's embedding a removed or expired video. This prevents the watch page from being indexable.robots`noindex`

`meta`

tag**Indicate an expiration date**in your structured data (theproperty) or video sitemap (use the`expires`

`<video:expiration_date>`

element). Here is an example of a video sitemap with a video that expired in November 2009:<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9" xmlns:video="http://www.google.com/schemas/sitemap-video/1.1"> <url> <loc>https://example.com/videos/some_video_landing_page.html</loc> <video:video> <video:thumbnail_loc> https://example.com/thumbs/123.jpg </video:thumbnail_loc> <video:title> Grilling steaks for summer </video:title> <video:description> Bob shows you how to grill steaks perfectly every time </video:description> <video:player_loc> https://example.com/videoplayer?video=123 </video:player_loc>

**<video:expiration_date>2009-11-05T19:20:30+08:00</video:expiration_date>**</video:video> </url> </urlset>
...
[ meta tags](/search/docs/crawling-indexing/robots-meta-tag#unavailable-after).
...
If you use [ VideoObject structured data](/search/docs/appearance/structured-data/video)
to describe a video, set the

[property to specify which regions can get the video result. If you omit this property, all regions can see the video in search results.](/search/docs/appearance/structured-data/video#regions-allowed)

`regionsAllowed`

Alternatively, you can use the [ ineligibleRegion](/search/docs/appearance/structured-data/video#ineligible-region)
property to specify which regions can't get the video result.

#### Restrict using a video sitemap

In a video sitemap, the [ <video:restriction>](/search/docs/crawling-indexing/sitemaps/video-sitemaps#country-restriction)
tag can be used to allow or deny the video from appearing in specific countries. Only one

`<video:restriction>`

tag is allowed per video entry.
The `<video:restriction>`

tag must contain one or more space-delimited
[ISO 3166-1 two- or three-letter country codes](https://en.wikipedia.org/wiki/ISO_3166-1). The
required `relationship`

attribute specifies the type of restriction.

`relationship="allow"`

: The video can appear only for the specified countries. If no countries are specified, the video won't appear anywhere.`relationship="deny"`

: The video can appear everywhere except for the specified countries. If no countries are specified, the video will appear everywhere.
...
[Learn more about labeling SafeSearch pages](/search/docs/crawling-indexing/safesearch).
...
[Video indexing report](https://support.google.com/webmasters/answer/9495631): See how many of your indexed watch pages contain an indexed video, and see reasons for why other videos didn't get indexed.[Video rich result report](https://support.google.com/webmasters/answer/7552505): Review and fix issues with your`VideoObject`

structured data implementation.[Performance report](https://support.google.com/webmasters/answer/7576553#by_search_appearance&zippy=%2Csearch-appearance): Use the Videos search appearance filter to monitor how your videos are performing in Google Search.
