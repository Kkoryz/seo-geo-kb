...
To specify a [canonical URL](/search/docs/crawling-indexing/canonicalization) for
...
-
: A strong signal that the target of the redirect should become canonical.**Redirects** -
: A strong signal that the specified URL should become canonical.`rel="canonical"`

`link`

annotations -
: A weak signal that helps the URLs that are included in a sitemap become canonical.**Sitemap inclusion**
...
[Google will identify which version of the URL is objectively the best version to show to users in Search](/search/docs/crawling-indexing/canonicalization#canonical-how).
...
-
**To specify which URL that you want people to see in search results.**You might prefer people to reach your green dress product page through`https://www.example.com/dresses/green/green-dress.html`

rather than`https://example.com/dresses/cocktail?gclid=ABCD`

. -
**To consolidate signals for similar or duplicate pages.**It helps search engines to be able to consolidate the signals they have for the individual URLs (such as links to them) into a single, preferred URL. This means that signals from other sites to`https://example.com/dresses/cocktail?gclid=ABCD`

get consolidated with links to`https://www.example.com/dresses/green/green-dress.html`

if the latter becomes canonical. -
...
**We don't recommend**usingto prevent selection of a canonical page within a single site, because it will completely block the page from Search.`noindex`
...
If you're using
, make sure to specify a canonical page in the same language, or the best possible substitute language if a canonical page doesn't exist for the same language.`hreflang`

elements - When linking within your site, link to the canonical URL rather than a duplicate URL. Linking consistently to the URL that you consider to be canonical helps Google understand your preference.
...
element in the code for all duplicate pages,
...
HTTP headerSend a `rel="canonical"`
...
[Sitemap](#sitemap-method)Specify your canonical pages in a sitemap.
...
-
Less powerful signal to Google than the
`rel="canonical"` mapping technique.
|

[Redirects](#redirects-method)[AMP variant](/search/docs/crawling-indexing/amp)## Use `rel="canonical"`
...
annotations that suggest alternate versions of a page are ignored;
specifically, `rel="canonical"`
...
attributes are not used for
canonicalization. Instead, use the appropriate `link`

annotations to specify alternate
versions of a page; for example, `link`

`rel="alternate"`

[ hreflang](/search/docs/specialty/international/localized-versions) for
...
element (also known as a
*canonical element*) is an element used in the `head`
...
section of duplicate pages, pointing to the canonical page. For example:<html> <head> <title>Explore the world of dresses</title>

**<link rel="canonical" href="https://example.com/dresses/green-dresses" />**<!-- other elements --> </head> <!-- rest of the HTML --> -
If the canonical page has a mobile variant on a separate URL, add a
`rel="alternate"`

`link`

element to it, pointing to the mobile version of the page:<html> <head> <title>Explore the world of dresses</title>

**<link rel="alternate" media="only screen and (max-width: 640px)" href="https://m.example.com/dresses/green-dresses">**<link rel="canonical" href="https://example.com/dresses/green-dresses" /> <!-- other elements --> </head> <!-- rest of the HTML --> -
Add any
or other elements that are appropriate for the page.`hreflang`
...
element. Even though relative paths are supported by Google, they
...
section of the HTML, so make sure at least the
[ <head> section is valid HTML](/search/docs/crawling-indexing/valid-page-metadata).
...
element,
make sure to
[inject the canonical link element properly](/search/docs/crawling-indexing/javascript/javascript-seo-basics#properly-inject-canonical-links).
...
target attribute as defined by
...
their own URL, you can return a `rel="canonical"`

HTTP header to tell
Googlebot what is the canonical URL for the non-HTML files. For example, to indicate
that the PDF version of the `.docx`

version should be canonical, you might
add this HTTP header for the `.docx`
...
element, use absolute URLs
in the `rel="canonical"`
...
[sitemap](/search/docs/crawling-indexing/sitemaps/overview). All pages
...
[permanent redirection methods](/search/docs/crawling-indexing/301-redirects)
...
from the HTTP page to the HTTPS page. -
Implement
...
rather than the HTTPS version.`hreflang`
...
will be preferred as canonicals instead
of the `/de-at/`

page that doesn't appear in the
`hreflang`

cluster.

Read more about
[troubleshooting and fixing canonicalization issues](/search/docs/crawling-indexing/canonicalization-troubleshooting).
