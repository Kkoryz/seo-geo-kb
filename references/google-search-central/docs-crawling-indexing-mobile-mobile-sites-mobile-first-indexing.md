---
title: "Mobile-first Indexing Best Practices | Google Search Central | Documentation | Google for Developers"
source_url: https://developers.google.com/search/docs/crawling-indexing/mobile/mobile-sites-mobile-first-indexing
...
Google uses the mobile version of a site's content, crawled with the
[smartphone agent](/search/docs/crawling-indexing/overview-google-crawlers),
...
-
Serves the same HTML code on the same URL regardless of the users' device (for example, desktop, tablet, mobile, non-visual browser), but can display the content differently based on the screen size.**Responsive design:****Google recommends Responsive Web Design because it's the easiest design pattern to implement and maintain.** -
**Dynamic serving:**Uses the same URL regardless of device. This configuration relies onand the`user-agent`

sniffingto serve a different version of the HTML to different devices.`Vary: user-agent`

HTTP response header -
**Separate URLs:**Serves different HTML to each device, and on separate URLs. Like dynamic serving, this configuration relies on the`user-agent`
...
-
**Use the same robots**If you use a different robots`meta`

tags on the mobile and desktop site.`meta`

tag on the mobile site (especially the`noindex`

or`nofollow`

tags), Google may fail to crawl and index your page when your site is enabled for mobile-first indexing. -
**Don't lazy-load primary content upon user interaction.**Google won't load content that requires user interactions (for example, swiping, clicking, or typing) to load.[Make sure that Google can see lazy-loaded content](/search/docs/crawling-indexing/javascript/lazy-loading). -
**Let Google crawl your resources.**Some resources have different URLs on the mobile site from those on the desktop site. If you want Google to crawl your URLs, make sure that you're not blocking the URL with the.`disallow`
...
-
**Make sure that your mobile and desktop sites have the same structured data.**If you have to prioritize which types you add to your mobile site, start with,`Breadcrumb`
...
Make sure that the
[ title element](/search/docs/appearance/title-link#page-titles) and
the

[meta description](/search/docs/appearance/snippet#meta-descriptions)are equivalent across both versions of your site.

## Check the placement of your ads

Don't let ads harm your mobile page ranking. Follow the
[Better Ads Standard](https://www.betterads.org/standards/)
when displaying ads on mobile devices. For example, ads at the top of the page can take up too
...
[image best practices](/search/docs/appearance/google-images). In particular, we
recommend that you:

-
**Provide**Don't use images that are too small or have a low resolution on the mobile site.[high quality images](/search/docs/appearance/google-images#good-quality-photos). -
**Use a**Don't use unsupported formats or tags. For example, Google supports SVG format images, but our systems can't index a .jpg image in the[supported format for images](/search/docs/appearance/google-images#supported-image-formats).`<image>`

tag inside an inline SVG. -
...
**Make sure that the mobile site has the same alt text for images as the desktop site.**Use[descriptive alt text](/search/docs/appearance/google-images#descriptive-alt-text)for images on your mobile site as you do on your desktop site. -
**Make sure that the mobile page content quality is as good as the desktop page.**Use the same[titles, captions, filenames, and text](/search/docs/appearance/google-images#descriptive-titles-captions-filenames)relevant to the images on the mobile site as you do for the desktop site.

### Check your videos

Make sure that the videos on your mobile site follow the
[video best practices](/search/docs/appearance/video). In particular, we
recommend that you:
...
**Use a**for your videos and put videos in supported tags. Videos are identified in the page by the presence of an HTML tag, for example:[supported format](/search/docs/appearance/video#file-types)`<video>`
...
. -
**Use the same video structured data**on both your mobile site and desktop site. For more information,[check your structured data](#structured-data). -
...
**Make sure that your mobile version doesn't have URL fragments.**The fragment part of the URL is the end of the URL that starts with`#`

. Most of the time, URL fragments are not indexable, these pages will be missing from the index after your domain is enabled for mobile-first indexing. -
...
**Verify both versions of your site in**to make sure that you have access to data and messages for both versions. Your site may experience a data shift when Google switches to mobile-first indexing for your site.[Search Console](https://search.google.com/search-console) -
**Check**When you use`hreflang`

links on separate URLs.`rel=hreflang`

for`link`

elements[internationalization](/search/docs/specialty/international/managing-multi-regional-sites), link between mobile and desktop URLs separately. Your mobile URLs'`hreflang`

must point to mobile URLs, and similarly desktop URL`hreflang`
...
**Verify that your**work as you intend for both versions of your site. The robots.txt file lets you specify which parts of a website may be crawled or not. In most cases, use the same robots.txt rules for both mobile and desktop versions of your site.[robots.txt rules](/search/docs/crawling-indexing/robots/intro) -
**Use the correct**between your mobile and desktop versions. The desktop URL is always the canonical, and the mobile version is the alternate of that URL.`rel=canonical`

and`rel=alternate`

`link`

elementsHere's an example of
...
element that points to the desktop URL as the canonical URL.<link rel="canonical" href="https://example.com/">
...
element that points to itself as the canonical URL, followed by another`link`

element that points to the mobile version as the alternate version of this URL.<link rel="canonical" href="https://example.com/"> <link rel="alternate" media="only screen and (max-width: 640px)" href="https://m.example.com/">
...
[Data Highlighter](https://support.google.com/webmasters/answer/2692911)to provide structured data, regularly check the[Data Highlighter dashboard](https://www.google.com/webmasters/tools/data-highlighter)for extraction errors. -
Use the
[URL Inspection tool](https://support.google.com/webmasters/answer/9012289)to check that the content is visible on the rendered page (the rendered page is how Google sees your page).
|
|
error
...
-
Use the same robots
`meta` tags on the mobile site and the desktop site. Don't use the`nofollow` tag on the mobile page (otherwise, Google won't crawl and index the images on your page when your site is enabled for mobile-first indexing). -
Use a
[supported format and tag for images](/search/docs/appearance/google-images#supported-image-formats). For example, Google supports SVG format images, but our systems can't index a .jpg image in the`<image>` tag inside an inline SVG. -
Don't lazy-load primary content upon user interaction. Google won't load content that
requires user interactions (for example, swiping, clicking, or typing) to load.
[Make sure that Google can see lazy-loaded content](/search/docs/crawling-indexing/javascript/lazy-loading).
|
## Blocked image |
error
...
-
Don't let ads harm your mobile page ranking. Follow the
[Better Ads Standard](https://www.betterads.org/standards/)when displaying ads on mobile devices. - Make sure your mobile site contains the same content as your desktop site. If your mobile site has less content than your desktop site, consider updating your mobile site so that its primary content is equivalent with your desktop site. Only the content shown on the mobile site is used for indexing.
- Make sure you use the same clear and meaningful headings on the mobile site as you do on the desktop site.
-
Use the same
[titles, captions, filenames, and text](/search/docs/appearance/google-images#descriptive-titles-captions-filenames)relevant to the images on the mobile site you do for the desktop site.
|
## Video issues |
error
...
-
Use a
[supported format](/search/docs/appearance/video#file-types)for your videos and put them in supported tags. Videos are identified in the page by the presence of an HTML tag, for example:`<video>` ,`<embed>` , or`<object>` . -
Don't lazy-load primary content upon user interaction. Google won't load content that
requires user interactions (for example, swiping, clicking or typing) to load.
[Make sure Google can see lazy-loaded content](/search/docs/crawling-indexing/javascript/lazy-loading). - Place the video in an easy to find location on your mobile site. For example, it might harm the video's ranking if users need to scroll down too much to find the video on mobile page.
|
## Hostload issues |
error
