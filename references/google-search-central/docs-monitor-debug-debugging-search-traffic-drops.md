---
title: "Debug Google Search Traffic Drops | Google Search Central | Documentation | Google for Developers"
source_url: https://developers.google.com/search/docs/monitor-debug/debugging-search-traffic-drops
...
[Google Trends](https://trends.google.com/) to investigate the
...
algorithms accordingly; [core updates](/search/updates/core-updates) and other smaller
updates may change how some pages perform in Google Search results. We post about notable
improvements to our systems on our [list of ranking updates page](https://status.search.google.com/products/rGHU1u87FJnkP6W2GwMi/history);
...
For example, dropping from position 2 to 4.[Small drop in position?](#small)For example, dropping from position 4 to 29.[Large drop in position?](#large)
...
sure it's [helpful, reliable and people-first](/search/docs/fundamentals/creating-helpful-content).
...
example, a misplaced `noindex`

tag, which would depend on Google crawling the page,
meaning there would be a slower drop in traffic).

Check the [Crawl stats report](https://search.google.com/search-console/settings/crawl-stats) and
...
If your site is affected by a [security threat](/search/docs/monitor-debug/security),
...
automated systems and, as needed, human review that can result in a [manual action](https://search.google.com/search-console/manual-actions).
If your site doesn't comply with the [Spam policies for Google web search](/search/docs/essentials/spam-policies),
your content might rank lower in results or not appear in results at all.

If you suspect a drop due to a spam violation, review our [spam policies](/search/docs/essentials/spam-policies)
...
by [applying a filter](https://support.google.com/webmasters/answer/7576553#filteringdata)
to include only a query at a time (choose the queries that receive the most traffic); then, check
them on [Google Trends](https://trends.google.com/trends/) to
...
If you see a drop after moving and it's not recovering, check the [site move troubleshooting section](/search/docs/crawling-indexing/site-move-with-url-changes#troubleshooting)
...
If both impressions and clicks dropped, check the [list of the most common reasons](#main-causes) that
could have caused it. If your impressions remain the same but your clicks drop, you might not be
generating the best [page title](/search/docs/appearance/title-link)
and [snippet](/search/docs/appearance/snippet) that you could, and so users don't
understand the content of your page, or perhaps other sites had a more appealing
[rich result](/search/docs/appearance/structured-data/search-gallery).

Visit the [Search Performance report](https://search.google.com/search-console/performance/search-analytics)
and try [applying filters](https://support.google.com/webmasters/answer/7576553#filteringdata)
...
you could use the [Search Analytics API](https://developers.google.com/webmaster-tools/search-console-api-original/v3/searchanalytics) or
[bulk data exports](https://support.google.com/webmasters/answer/12917675)
...
[comparison filter](https://support.google.com/webmasters/answer/7576553#comparingdata)).
...
[self-assessing your content](/search/docs/fundamentals/creating-helpful-content#self-assess)
...
If it's a site-wide issue, check the [Page indexing report](https://search.google.com/search-console/index).
...
If you want to go the extra mile, you can [use Google Trends](/search/docs/monitor-debug/trends-start)
...
**Changing interests**. If there are major changes in what and how people search, people may start searching for different queries, or using their devices for different purposes. In addition, if you sell a specific brand online, there might be new products for the same search queries.. For example, the[Seasonality](https://en.wikipedia.org/wiki/Seasonality)[rhythm of food website](https://rhythm-of-food.net/)shows that food related queries are very seasonal: people search for diets in January, turkey in November, and champagne in December. Different industries have different levels of seasonality.
...
([check the data](https://trends.google.com/trends/explore?date=today%205-y&geo=US&q=turkey,chicken,coffee)):
