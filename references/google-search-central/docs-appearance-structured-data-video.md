---
title: "Video (VideoObject, Clip, BroadcastEvent) Schema Markup | Google Search Central | Documentation | Google for Developers"
...
# Video (VideoObject, Clip, BroadcastEvent) Schema Markup | Google Search Central | Documentation | Google for Developers
...
your video with [ VideoObject](#video-object). Adding video structured data
to your

[watch pages](/search/docs/appearance/video#watch-page)can also make it easier for Google to find your video. Videos can appear in several different places on Google, including the main search results page, Video mode, Google Images, and
...
Here's an example of [ VideoObject](#video-object) and
...
Here's an example of [ VideoObject](#video-object)
and
...
[additional properties](#seek)needed for
...
For your video structured data to be eligible for usage in Google Search, you must follow the
...
and [video indexing requirements](/search/docs/appearance/video#indexing-criteria).

In addition, we recommend that you check out these guidelines if they apply to your video content:

[Livestream guidelines](#livestream-guidelines)`Clip`

and`SeekToAction`

guidelines[Best practices for marking timestamps on YouTube](#best-practices-youtube)

### LIVE badge guidelines

If you're adding [ BroadcastEvent](#broadcast-event) to livestream
...
[Indexing API](/search/apis/indexing-api/v3/quickstart). Call the API for the following events:- When the video goes live
...
If you're adding [ Clip](#clip) or

[structured data to mark video segments, follow these guidelines:](#seek)
...
structured data documentation.`VideoObject`

**For**: Make sure that no two clips on the same video defined on the same page share a start time.`Clip`

structured data only**For**: Google must be able to`SeekToAction`

structured data only[fetch your video content files](/search/docs/appearance/video#allow-fetch).

## Structured data type definitions

This section describes the structured data types related to video features in Google Search.
You must include the required [ VideoObject](#video-object) properties for your
markup to be eligible for usage in Google Search. You can also include the
recommended properties to add more information about your

`VideoObject`

, which could provide a better
user experience. In addition to [, you can add the following data types to enable video enhancements in Google Search:](#video-object)

`VideoObject`

: Mark up livestream videos to enable a LIVE badge on your video.`BroadcastEvent`

: Mark important segments in your video to help users quickly navigate to specific points in a video.`Clip`

: Enable key moments by indicating how your URL structure works, so that Google can automatically identify key moments and link users to those points within the video.`SeekToAction`
...
`thumbnailUrl` |
Repeated A URL pointing to the video's unique thumbnail image file. Follow the |
`uploadDate` |
The date and time the video was first published, in
|
...
`contentUrl`
|
A URL pointing to the video file's actual content bytes, in one of the
"contentUrl": "https://www.example.com/video/123/file.mp4" Make sure to follow our
|
`description` |
...
the "embedUrl": "https://www.example.com/embed/123" Make sure to follow our
|
`expires` |
If applicable, the date and time after which the video will no longer be available, in
|
`hasPart`
|
If your video has important segments, nest the `VideoObject` . For example:<script type="application/ld+json"> { "@context": "https://schema.org/", "@type": "VideoObject", "name": "Cat video", |
`ineligibleRegion`
|
...
the `VideoObject` . For example:<script type="application/ld+json"> { "@context": "https://schema.org/", "@type": "VideoObject", "name": "Cat video", |
`regionsAllowed`
|
...
properties in your [VideoObject](#video-object). While
`BroadcastEvent`
...
It's required to provide the If |
...
[ VideoObject](/search/docs/appearance/structured-data/video#video-object). While
...
is
available at [schema.org/Clip](https://schema.org/Clip).
...
[ VideoObject](#video-object). While
...
is
available at [schema.org/SeekToAction](https://schema.org/SeekToAction).
...
|
The placeholder string that Google will identify as your timestamp structure and then replace with the number of seconds to skip to. Use the following value: "startOffset-input": "
|
`potentialAction.target`
|
The URL of the page that contains this {seek_to_second_number} For example, replace the timestamp part of the URL: "target": "https://video.example.com/watch/videoID?t= So that the timestamp now looks like this: "target": "https://video.example.com/watch/videoID?t= |
