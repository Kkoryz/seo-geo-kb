...
source_url: https://ahrefs.com/blog/how-ahrefs-counts-links/
...
[Links](https://ahrefs.com/blog/what-are-backlinks/) take users from one webpage to another when clicked. There are many ways to create them, with the most common method being the classic HTML `<a>`
...
Links from one website to another created using the classic HTML `<a>`
...
However, we only count links inserted with JavaScript if they are in the format of an HTML `<a>`

element with an href attribute. You’ll see these links tagged in the backlinks report as “JS,” like this:
...
something with noindex will never reach the serving index, but we will have the fetched copy for things like link graph calculation.— Gary 鯨理／경리 Illyes (@methode)
